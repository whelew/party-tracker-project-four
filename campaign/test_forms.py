...
class TestCharacterForm(TestCase):
    def setUp(self):
        ...
        self.campaign = Campaign.objects.create(
            name='Testcampaign4',
            description='This is test four', user=self.user)
    ...
    def test_if_data_is_valid(self):
        ...
        for attribute in [
            ...
            self.assertFalse(
                form.is_valid(),
                msg=f'Form Is Not Valid {attribute} is higher than 20')
            invalid_attr[attribute] = 0
            form = CharacterForm(data=invalid_attr)
            self.assertFalse(
                form.is_valid(),
                msg=f'Form Is Not Valid when {attribute} is lower than 1')
        ...
        for field in ['name',
            ...
            self.assertFalse(
                form.is_valid(),
                msg=f'Form Is Not Valid when {field} is empty')
        ...
        for validator in [0, 2001]:
            ...
            self.assertFalse(
                form.is_valid(),
                msg=f'Form Is Not Valid if health is {validator}')
