...
class CampaignTest(TestCase):
    ...
    def create_campaign_post(self):
        ...
        self.assertEqual(current_campaign.description,
                         'This is my second test campaign')
    ...
    def test_campaign_info(self):
        ...
        current_campaign = Campaign.objects.get(
            pk=self.campaign.id, user=self.user)
    ...
    def test_campaign_info_no_characters(self):
        ...
        empty_campaign = Campaign.objects.create(
            name='Empty', description='No characters', user=self.user)
    ...
    def test_create_character_post(self):
        ...
        current_characters = Character.objects.filter(
            campaign_id=self.campaign).last()
        ...
        self.assertRedirects(response, reverse(
            'campaign_info', kwargs={'pk': self.campaign.id}))
    ...
    def test_invalid_stat_increment(self):
        for action, expected_value, attribute in [
            ('increment', 20, 'wisdom'),
            ('decrement', 1, 'dexterity')
        ]:
            url = reverse('update_character_stat',
                          kwargs={
                              'character_id': self.character.id,
            ...
            response = self.client.post(url)
            self.assertEqual(
                 getattr(self.character, attribute), expected_value)
            self.assertRedirects(response, reverse(
                'campaign_info', kwargs={'pk': self.campaign.id}))
